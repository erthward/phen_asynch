...
var calcMinPosNIRv = function(nYears,
                              endYear,
                              dayStep
                             ){
  ...
  var minPosNIRv = NIRv
    ...
    // NOTE: rename to just 'NIRv', so that it matches our band in the main dataset, 
    //       where we'll be using this to replace negative values
    .rename('NIRv')
    .reproject({crs: SIF_proj, scale: SIF_scale});
  return minPosNIRv;
};

var minPosNIRv = calcMinPosNIRv(params.NIRvNYears,
                                params.NIRvEndYear,
                                params.NIRvDayStep
                               );
...
if (params.map_intermediates){
  print(minPosNIRv);
  print(minPosNIRv.projection().nominalScale().getInfo());
  Map.addLayer(minPosNIRv);
}


// export as a masking layer
Export.image.toAsset({image: minPosNIRv,
                      description: 'NIRv_min_pos',
                      assetId: 'users/drewhart/NIRv_min_pos',
                      region: params.roi,

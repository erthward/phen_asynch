...
6. *On laptop*, run `python src/asynch/anal/plot_flowphen_and_landgen_results.py` to execute both landscape genetic analyses (*R. granulosa* and *X. fuscus*) and then visualize results and example taxa from both the iNaturalist flowering phenology peak-fitting and MMRR analyses and the landscape genetic analyses (**1 task, <1h runtime**).
7. *On laptop*, copy the figure output by the previous step into `res/figs/FIG_flowphen_and_landgen_analysis_results.odg` using LibreOffice Draw, add the letters, borders, and other annotations, save the file, then save a static image of the file (by hitting \<Ctrl>+A to select all, clicking "File" > "Export...", checking the 'Selection' checkbox in the bottom left corner, selecting .png as the file type, clicking "Save", clicking the "Modify resolution" radio-button on the pop-up and setting the resolution to 700 pixels/inch, then clicking the "Modify dimensions" radio button, setting the width to 8 inches and letting the height auto-adjust, then clicking "OK").

...
3. *On Savio*, run `bash src/asynch/anal/drivers/run_rf/run_ch3_rf_job.sh` to load the conda environment that has R v4.0.3 and the necessray packages, then run the sbatch job script that loops over vars (NIRv, SIF) and neighborhood radii, each time prepping data layers, running the random forest analysis, and generating identical results (**1 task, ~18h runtime**).

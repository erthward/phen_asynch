...
The study does the following main things:

1. presents a simple method for near-global, consistent estimation of characteristic, annual land surface phenology <span style="color:gray;">(*plain English: presents a technique to estimate, nearly everywhere, the average timing of seasonal plant growth cycles*)</span>
2. analyzes the patterns of global diversity and regional heterogeneity that result from application of that method to 20 years of MODIS near infrared reflectance of vegetation ($NIR_{V}$) data <span style="color:gray;">(*plain English: applies that technique to map plant seasonality nearly everywere, using pictures of Earth collected from satellites, in space!*)</span>
3. maps spatial asynchrony of LSP and assesses its potential drivers <span style="color:gray;">(*plain English: makes a map showing places where seasonal timing is very different in some places than in other not-too-far-away places, and another map trying to figure out why that happens in the places where it does*)</span>
4. describes a latitudinal gradient in isoclimatic phenological asynchrony <span style="color:gray;">(*plain English: finds that sometimes, mainly in the tropics, the seasonal timing can be pretty different between not-too-far-away sites even if they have similar overall climates --- for example, if they're both the same type of rainforest*)</span>
5. uses a number of ground datasets to demonstrate that remotely sensed LSP maps can predict patterns of phenological asynchrony and genetic divergence across a range of species and regions <span style="color:gray;">(*plain English: shows that maps of plant seasonality based on satellite pictures can be used to predict how the timing of plant and animal reproduction differs between different places, and thus how related the plants and animal are between those places*)</span>
...
available as a Google Earth Engine asset ('projects/lyrical-ring-231401/assets/Terasaki_Hart_2025_LSP_main_map_results').
...
1. *On GEE*, run `src/asynch/anal/drivers/prep_data/calc_veg_entropy.js` to produce the vegetation cover entropy map that will be used as a covariate in the phenological asynchrony predictive model. (**1 task, <10m runtime**)
2. *On GEE*, run `src/asynch/anal/drivers/prep_data/calc_lulcc_percent.js` twice, first to aggregate the [Hansen et al. 2022](https://iopscience.iop.org/article/10.1088/1748-9326/ac46ec/meta) global land use and land cover dataset to 1km, then to further aggregate it and summarize it to produce a map of the each pixel's neighborhood mean of the taret-res pixels' percent of subpixels exhibiting LULCC, to be used as a covariate in the phenological asynchrony predictive model. (**2 tasks, ~40m and <10m runtimes**)
3. *On GEE*, run `src/asynch/anal/drivers/prep_data/calc_fire_frequency.js` to produce a map of neighborhood mean fire frequency, to be used as a covariate in the phenological asynchrony predictive model. (**1 task, ~10m runtime**)
4. *On laptop*, manually download the GMTED-derived, 100-km median-aggregated vector ruggedness metric (VRM) dataset from [EarthEnv](http://www.earthenv.org/topography) (file 'vrm_100KMmd_GMTEDmd.tif'), then uploade to the 'LSP_ancillary_datasets' folder on Google Drive (**1 task, <10s runtime**)
5. *On Savio*, run `bash src/asynch/anal/drivers/prep_data/dl_asynch_drivers_files_from_bdrive.sh` to download both of these covariate maps into the correct directory (**1 task, <1m runtime**).
6. *On Savio*, run `python src/asynch/anal/drivers/prep_data/flip_lulcc_raster.py` to flip the y-dimension CRS metadata on the neighborhood-mean LULCC raster (written out by GEE upside-down, perhaps becaue of inheriting projection from original asset it reads) (**1 task, <1m runtime**).
7. *On Savio*, run `bash src/asynch/anal/drivers/prep_data/cp_asynch_drivers_files_to_rf_data.sh` to copy all asynch maps into the random forest-modelling data directory (**1 task, <1m runtime**).
...
2. *On Savio*, run `conda activate asynch` to use the R v4.0.3 environment we've been using since the start, run `srun ...` to create an interactive job on the savio3 partition, then start an R session and interactively run the code in `src/asynch/anal/drivers/run_rf/run_phen_asynch_rf.r`, with `asynch.var` set to 'NIRv', `neigh.rad` set to '100', and `coords.as.covars` set to 'y' (by switching the commenting on the corresponding lines at the top of the file), to execute the random forest analysis on the main phenological asynchrony dataset (NIRv-based LSP asynchrony calculated within 100 km radial neighborhoods) and save hyperparameter tuning results. (Be sure to interactively execute the code blocks captured by the `if (F){ ... }` blocks, which will save the hyperparameter-tuning results as CSVs and the Boruta feature selection results as a boxplot PNG.) Manually inspect the results, then use them to set the hyperparameters and variable inclusion to be used in all 12 random forest models (2 LSP datasets × 3 neighborhood radii × 2 options for including/excluding geographic coordinates as covariates) (set these in the code block starting at line 410 and 486) (**1 task, ~3h interactive runtime**).
3. *On Savio*, call `conda activate asynch` to load the conda environment that has R v4.0.3 and the necessary packages, then run `sbatch src/asynch/anal/drivers/run_rf/ch3_rf_job.sh` to loop over vars (NIRv, SIF) and neighborhood radii, each time prepping data layers, running the random forest analysis, and generating identical results (**1 task, ~27h runtime**).
4. *On Savio*, run `sbatch src/asynch/anal/drivers/summ_results/ch3_rasterize_SHAP_job.sh` to convert output CSVs of global SHAP values to GeoTIFFs (**1 task, ~9h runtime**).
5. *On Savio*, run `sbatch src/asynch/anal/drivers/summ_results/ch3_rasterize_err_job.sh` to convert output CSVs of global RF prediction errors to GeoTIFFs (can run synchronously with previous step) (**1 task, ~1h runtime**).
...
2. *On GEE*, upload the resulting GeoTIFF as a new asset, setting the Asset ID to 'Terasaki_Hart_2024_LSP_main_map_results' in the cloud project 'lyrical-ring-231401', setting the pyramiding policy to MEAN (works fine for every layer except the last one, which is not of major interest), and setting the masking mode to None (because missing values are already NaN within the file) (**1 task, ~30m runtime**).

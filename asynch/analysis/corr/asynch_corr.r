...
if (F){
  for (subset.frac in subset.fracs){
    ...
    write.csv(as.data.frame(hyper_grid),
              paste0(data.dir, 'tuning_results_subset_frac_', subset.frac, '.csv'),
              row.names=F)
  }
}
...
if (F){
  ...
  ggsave(global_main_plots, file=paste0(data.dir, 'global_grrf_main_plot.png'),
...
write.csv(shap_full, paste0(data.dir, 'rf_SHAP_vals.csv'), row.names=F)
shap_full_w_coords = cbind(df_full_unproj[, c('x', 'y')], shap_full)
write.csv(shap_full_w_coords, paste0(data.dir, 'rf_SHAP_vals_w_coords.csv'), row.names=F)

...
def calc_time_series(coeffs, design_mat):
    """
...
# Make the coeff-dist, seasonal-dist comparison plot
dists_seas = []
dists_coef = []
std = 10000
for i in range(100000):
    # draw a random vector of 5 coeffs
    # TODO: MAKE SURE DRAWING FROM THE FULL RANGE OF COEFFS SEEN IN MY DATA!!
    coeffs1 = np.random.normal(0, std, 5)
    # and a second
    coeffs2 = np.random.normal(0, std, 5)
    # calculate the euc dist between the seasonal patterns of each
    dist_seas = calc_euc_dist(calc_time_series(coeffs1, DESIGN_MAT),
                              calc_time_series(coeffs2, DESIGN_MAT))
    # and calculate the euc dist between the coeff vectors
    dist_coef = calc_euc_dist(coeffs1, coeffs2)
    # save those dists
    dists_seas.append(dist_seas)
    dists_coef.append(dist_coef)

fig, ax = plt.subplots(1)
plt.plot(dists_seas, dists_coef, '.k', alpha=0.01)
fig.show()
ax.set_xlabel('Euclidean distance between fitted 1-year seasonal functions',
              fontsize=12)
ax.set_ylabel('Euclidean distance between coefficients', fontsize=12)
ax.set_title(('Comparison of Euclidean distances between fitted seasonal '
              'patterns and the coefficients they\'re derived from '
              '\n(1e5 randomly drawn pairs of coefficients ~N(0,%i) simulating '
              'pairwise pixel comparisons)') % std, fontsize=14)
fig.show()
fig.savefig('seasonal_coeff_dist_comparison_std-%i.png' % std, dpi=500)
